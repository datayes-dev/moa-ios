...
+ (NSString *)deviceId
{
    NSString *deviceIdString = [SSKeychain passwordForService:@"com.datayes.ira" account:@"deviceId"];
    
    if ([deviceIdString length] <= 0) {
        deviceIdString = [DYTools createUUID];
        [SSKeychain setPassword:deviceIdString forService:@"com.datayes.ira" account:@"deviceId"];
    }
